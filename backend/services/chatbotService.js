...
class ChatbotService {
  ...
  // Enhanced intent analysis with SambaNova AI + rule-based fallback + context awareness
  async analyzeIntent(message, context = {}) {
    ...
    try {
      // Try SambaNova AI intent detection first
      const sambaNovaResult = await sambaNovaService.analyzeIntent(message, {
        sessionId,
        userId,
        conversationHistory: conversationContext?.recentMessages || []
      });
      
      if (sambaNovaResult && sambaNovaResult.confidence > 0.7) {
        console.log('✅ Using SambaNova AI intent analysis:', sambaNovaResult.intent);
  ...
  async generateResponse(intent, entities, userId = null, context = {}) {
    ...
    try {
      ...
      if (intent.source === 'sambanova' || intent.confidence > 0.8) {
        const sambaNovaResponse = await sambaNovaService.generateResponse(intent, entities, {
          ...context,
          originalMessage,
          conversationHistory: conversationContext?.recentMessages || [],
  ...
  async enhanceSambaNovaResponse(sambaNovaResponse, intent, entities, userId, context = {}) {
    // Start with SambaNova's generated message
    let baseMessage = sambaNovaResponse.message || this.generateDefaultMessage(intent.intent, entities);
